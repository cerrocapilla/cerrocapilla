...
// ============================================================
// 25.1 FUNCIÓN PARA ESCUCHAR CAMBIOS EN FIRESTORE
// ============================================================
...
function escucharCambiosFirebase() {
    ...
    try {
        ...
        onSnapshot(qEventos, (snapshot) => {
            snapshot.docChanges().forEach((change) => {
                if (change.type === 'added') {
                    const nuevoEvento = change.doc.data();
                    console.log('📅 Nuevo evento detectado:', nuevoEvento.nombre);
                    
                    // Verificar si el usuario ya lo vio
                    if (!leidos.eventos.has(parseInt(nuevoEvento.id))) {
                        // Agregar a eventos si no existe
                        if (!eventos.find(e => e.id === parseInt(nuevoEvento.id))) {
                            eventos.push({
                                id: parseInt(nuevoEvento.id),
                            ...
                            // Recargar sección si está visible
        ...
        onSnapshot(qNovedades, (snapshot) => {
            snapshot.docChanges().forEach((change) => {
                if (change.type === 'added') {
                    const nuevaNovedad = change.doc.data();
                    console.log('📰 Nueva novedad detectada:', nuevaNovedad.nombre);
                    
                    if (!leidos.novedades.has(parseInt(nuevaNovedad.id))) {
                        if (!novedades.find(n => n.id === parseInt(nuevaNovedad.id))) {
                            novedades.push({
                                id: parseInt(nuevaNovedad.id),
        ...
        onSnapshot(qNoticias, (snapshot) => {
            snapshot.docChanges().forEach((change) => {
                if (change.type === 'added') {
                    const nuevaNoticia = change.doc.data();
                    console.log('📢 Nueva noticia detectada:', nuevaNoticia.nombre);
                    
                    if (!leidos.noticias.has(parseInt(nuevaNoticia.id))) {
                        if (!noticias.find(n => n.id === parseInt(nuevaNoticia.id))) {
                            noticias.push({
                                id: parseInt(nuevaNoticia.id),
        ...
        console.log('✅ Listener de Firebase activado para eventos, novedades y noticias');
...
console.log('✅ Sistema de notificaciones en tiempo real con Firebase configurado');
