...
// ============================================================
//  25. CONEXIÓN CON FIREBASE REALTIME DATABASE
// ============================================================
...
function conectarFirebase() {
    ...
    onValue(eventosRef, (snapshot) => {
        ...
        if (data) {
            Object.values(data).forEach(nuevoEvento => {
                const id = parseInt(nuevoEvento.id);
                if (!leidos.eventos.has(id) && !eventos.find(e => e.id === id)) {
                    eventos.push({
                        id: id,
                        nombre: nuevoEvento.nombre || 'Nuevo evento',
                        categoria: 'Evento',
                        direccion: nuevoEvento.direccion || '',
                        coordenadas: nuevoEvento.coordenadas || [-30.8580, -64.5240],
                        telefono: nuevoEvento.telefono || null,
                        whatsapp: nuevoEvento.whatsapp || null,
                        instagram: nuevoEvento.instagram || null,
                        facebook: nuevoEvento.facebook || null,
                        twitter: nuevoEvento.twitter || null,
                        website: nuevoEvento.website || null,
                        delivery: false,
                        deliveryInfo: null,
                        horario: nuevoEvento.horario || '',
                        descripcion: nuevoEvento.descripcion || '',
                        metodoPago: [],
                        foto: nuevoEvento.foto || '',
                        fecha: nuevoEvento.fecha || 'Nuevo'
                    });
    ...
    onValue(novedadesRef, (snapshot) => {
        const data = snapshot.val();
        if (data) {
            Object.values(data).forEach(nuevaNovedad => {
                const id = parseInt(nuevaNovedad.id);
                if (!leidos.novedades.has(id) && !novedades.find(n => n.id === id)) {
                    novedades.push({
                        id: id,
                        nombre: nuevaNovedad.nombre || 'Nueva novedad',
                        categoria: 'Novedad',
                        direccion: nuevaNovedad.direccion || '',
                        coordenadas: nuevaNovedad.coordenadas || [-30.8580, -64.5240],
                        telefono: nuevaNovedad.telefono || null,
                        whatsapp: nuevaNovedad.whatsapp || null,
                        instagram: nuevaNovedad.instagram || null,
                        facebook: nuevaNovedad.facebook || null,
                        twitter: nuevaNovedad.twitter || null,
                        website: nuevaNovedad.website || null,
                        delivery: false,
                        deliveryInfo: null,
                        horario: nuevaNovedad.horario || null,
                        descripcion: nuevaNovedad.descripcion || '',
                        metodoPago: [],
                        foto: nuevaNovedad.foto || '',
                        fecha: nuevaNovedad.fecha || 'Nuevo'
                    });
    ...
    onValue(noticiasRef, (snapshot) => {
        const data = snapshot.val();
        if (data) {
            Object.values(data).forEach(nuevaNoticia => {
                const id = parseInt(nuevaNoticia.id);
                if (!leidos.noticias.has(id) && !noticias.find(n => n.id === id)) {
                    noticias.push({
                        id: id,
                        nombre: nuevaNoticia.nombre || 'Nueva noticia',
                        categoria: 'Noticia',
                        direccion: nuevaNoticia.direccion || '',
                        coordenadas: nuevaNoticia.coordenadas || [-30.8580, -64.5240],
                        telefono: nuevaNoticia.telefono || null,
                        whatsapp: nuevaNoticia.whatsapp || null,
                        instagram: nuevaNoticia.instagram || null,
                        facebook: nuevaNoticia.facebook || null,
                        twitter: nuevaNoticia.twitter || null,
                        website: nuevaNoticia.website || null,
                        delivery: false,
                        deliveryInfo: null,
                        horario: nuevaNoticia.horario || null,
                        descripcion: nuevaNoticia.descripcion || '',
                        metodoPago: [],
                        foto: nuevaNoticia.foto || '',
                        fecha: nuevaNoticia.fecha || 'Nuevo'
                    });
...
window.testAgregarEvento = async function() {
    ...
    try {
        const eventosRef = ref(window.firebaseDB, 'eventos/' + Date.now());
...
window.testAgregarNovedad = async function() {
    ...
    try {
        const novedadesRef = ref(window.firebaseDB, 'novedades/' + Date.now());
        await set(novedadesRef, {
            id: Date.now(),
        ...
        console.log('✅ Novedad de prueba agregada');
        return true;
    } catch (error) {
...
window.testAgregarNoticia = async function() {
    ...
    try {
        const noticiasRef = ref(window.firebaseDB, 'noticias/' + Date.now());
        await set(noticiasRef, {
            id: Date.now(),
        ...
        console.log('✅ Noticia de prueba agregada');
        return true;
    } catch (error) {
