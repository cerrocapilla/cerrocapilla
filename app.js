...
// ============================================================
//  24. SISTEMA DE NOTIFICACIONES EN TIEMPO REAL
// ============================================================

let ultimaActualizacion = 0;
const INTERVALO_BASE = 3000;
...
function escucharCambiosFirebase() {
    ...
    try {
        ...
        onSnapshot(qNovedades, (snapshot) => {
            snapshot.docChanges().forEach((change) => {
...
window.probarConexionFirebase = function() {
    console.log('🧪 Probando conexión con Firebase...');
    
    if (!window.firebaseDB) {
        console.error('❌ Firebase DB no existe');
        return false;
    }
    
    if (!window.firebaseHelpers) {
        console.error('❌ Firebase Helpers no existen');
        return false;
    }
    
    if (!window._firebaseListenerActivo) {
        console.warn('⚠️ Listener NO está activo, conectando ahora...');
        escucharCambiosFirebase();
        window._firebaseListenerActivo = true;
    }
    
    console.log('✅ Firebase conectado y escuchando');
    return true;
};

setTimeout(() => {
    window.probarConexionFirebase();
}, 5000);
