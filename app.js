...
// ============================================================
// CORRECCIÓN: CARGAR LEÍDOS ANTES DE CALCULAR NOTIFICACIONES
// ============================================================

// Cargar leídos INMEDIATAMENTE antes de cualquier cálculo
function cargarLeidosInmediatamente() {
    try {
        const data = JSON.parse(localStorage.getItem('capilla_leidos_v2') || '{}');
        
        if (data && typeof data === 'object') {
            if (data.eventos && Array.isArray(data.eventos)) {
                data.eventos.forEach(id => leidos.eventos.add(parseInt(id)));
            }
            if (data.novedades && Array.isArray(data.novedades)) {
                data.novedades.forEach(id => leidos.novedades.add(parseInt(id)));
            }
            if (data.noticias && Array.isArray(data.noticias)) {
                data.noticias.forEach(id => leidos.noticias.add(parseInt(id)));
            }
        }
        
        console.log('✅ Leídos cargados INMEDIATAMENTE:', {
        ...
        console.error('❌ Error cargando leídos inmediatamente:', e);
        return false;
    }
}

// LLAMAR INMEDIATAMENTE
cargarLeidosInmediatamente();

// MODIFICAR recalcularTodasLasNotificaciones para usar los leídos cargados
const recalcularOriginal = recalcularTodasLasNotificaciones;
recalcularTodasLasNotificaciones = function() {
    // Verificar que los leídos estén cargados
    if (leidos.eventos.size === 0 && leidos.novedades.size === 0 && leidos.noticias.size === 0) {
        cargarLeidosInmediatamente();
    }
...
// ============================================================
//  24. SISTEMA DE NOTIFICACIONES EN TIEMPO REAL - VERSIÓN DEFINITIVA
// ============================================================

console.log('⏰ ACTIVANDO SISTEMA DE NOTIFICACIONES EN TIEMPO REAL...');
...
const INTERVALO_BASE = 3000; // 3 segundos

// ============================================================
// 24.2 FUNCIÓN PRINCIPAL DE ACTUALIZACIÓN
// ============================================================
...
function actualizarNotificacionesEnTiempoReal() {
    ...
    if (ahora - ultimaActualizacion < INTERVALO_BASE) {
        return;
    }
    ...
    if (huboCambios) {
        console.log('🔄 Notificaciones actualizadas en tiempo real:', notificacionesEstado);
        
        // Mostrar toast si aumentó alguna
        if (notificacionesEstado.eventos > estadoAnterior.eventos) {
            mostrarNotificacionToast('🔔 ¡Nuevas notificaciones!');
        }
        if (notificacionesEstado.novedades > estadoAnterior.novedades) {
            mostrarNotificacionToast('🔔 ¡Nuevas notificaciones!');
        }
        if (notificacionesEstado.noticias > estadoAnterior.noticias) {
            mostrarNotificacionToast('🔔 ¡Nuevas notificaciones!');
        }
...
// ============================================================
// 24.3 MÚLTIPLES ESTRATEGIAS PARA GARANTIZAR ACTUALIZACIÓN
// ============================================================

// Estrategia 1: Intervalo fijo (siempre activo)
setInterval(() => {
    actualizarNotificacionesEnTiempoReal();
}, INTERVALO_BASE);

// Estrategia 2: Cuando la página se vuelve visible
document.addEventListener('visibilitychange', () => {
    if (!document.hidden) {
        console.log('👁️ Página visible - actualizando...');
...
// Estrategia 3: Cuando el usuario interactúa (click, touch, scroll)
const eventosInteraccion = ['click', 'touchstart', 'touchmove', 'scroll', 'focus'];
eventosInteraccion.forEach(evento => {
    document.addEventListener(evento, () => {
        // Solo si pasaron más de 2 segundos desde la última interacción
        if (Date.now() - ultimaActualizacion > 2000) {
            actualizarNotificacionesEnTiempoReal();
        }
    }, { passive: true });
});

// Estrategia 4: Cuando hay cambios en el DOM (para móvil)
...
// Estrategia 5: Cuando el usuario vuelve a la pestaña
window.addEventListener('focus', () => {
    console.log('🔍 Página enfocada - actualizando...');
    ultimaActualizacion = 0;
    setTimeout(() => {
        actualizarNotificacionesEnTiempoReal();
    }, 100);
...
// Estrategia 6: Cuando se reanuda desde el fondo (móvil)
if ('onresume' in document) {
    document.addEventListener('resume', () => {
        console.log('📱 App reanudada - actualizando...');
        ultimaActualizacion = 0;
        setTimeout(() => {
            actualizarNotificacionesEnTiempoReal();
        }, 200);
...
// Estrategia 7: Usar Page Visibility API (más confiable)
...
// ============================================================
// 24.4 INTERCEPTAR TODOS LOS MÉTODOS DE AGREGADO
// ============================================================

// Función para marcar nuevo elemento como no leído
function marcarNuevoElemento(seccion, id) {
    if (leidos[seccion] && leidos[seccion].has(id)) {
...
// Interceptar push en eventos
const pushEventosOriginal = eventos.push;
eventos.push = function(...items) {
    const result = pushEventosOriginal.apply(this, items);
    items.forEach(item => {
        if (item && item.id) {
            marcarNuevoElemento('eventos', item.id);
            mostrarNotificacionToast('🔔 ¡Nuevas notificaciones!');
        }
    });
    return result;
...
// Interceptar push en novedades
const pushNovedadesOriginal = novedades.push;
novedades.push = function(...items) {
    const result = pushNovedadesOriginal.apply(this, items);
    items.forEach(item => {
        if (item && item.id) {
            marcarNuevoElemento('novedades', item.id);
            mostrarNotificacionToast('🔔 ¡Nuevas notificaciones!');
        }
    });
    return result;
...
// Interceptar push en noticias
const pushNoticiasOriginal = noticias.push;
noticias.push = function(...items) {
    const result = pushNoticiasOriginal.apply(this, items);
    items.forEach(item => {
        if (item && item.id) {
            marcarNuevoElemento('noticias', item.id);
            mostrarNotificacionToast('🔔 ¡Nuevas notificaciones!');
        }
    });
    return result;
...
console.log('✅ Interceptores de push activados');

// ============================================================
// 24.5 FUNCIÓN PARA MOSTRAR TOAST (CORREGIDA)
// ============================================================

function mostrarNotificacionToast(mensaje) {
    console.log(`🔔 TOAST: ${mensaje}`);
    
    // Siempre usar mensaje genérico
    mensaje = '🔔 ¡Nuevas notificaciones!';
...
// ============================================================
// 24.6 FUNCIÓN PARA GUARDAR ESTADO
// ============================================================
...
// ============================================================
// 24.7 FUNCIONES DE UTILIDAD PARA DEBUG
// ============================================================

window.actualizarNotificacionesAhora = function() {
...
window.verEstadoNotificaciones = function() {
...
console.log('🔄 Inicializando sistema en tiempo real...');
...
// Forzar otra actualización después de 2 segundos
setTimeout(() => {
    console.log('🔍 Verificación secundaria...');
    actualizarNotificacionesEnTiempoReal();
}, 2000);

// Y otra después de 5 segundos (por si el navegador es lento)
setTimeout(() => {
    console.log('🔍 Verificación terciaria...');
    actualizarNotificacionesEnTiempoReal();
}, 5000);

console.log('✅ SISTEMA EN TIEMPO REAL ACTIVADO');
console.log('📊 Intervalo de actualización: cada 3 segundos');
console.log('🔄 Se actualiza automáticamente al interactuar con la página');
console.log('💡 Comandos útiles:');
console.log('  - actualizarNotificacionesAhora() : Forzar actualización manual');
console.log('  - verEstadoNotificaciones() : Ver estado actual');

// ============================================================
//  25. CONEXIÓN CON FIREBASE - NOTIFICACIONES EN TIEMPO REAL
// ============================================================

console.log('🔌 Conectando con Firebase para notificaciones en tiempo real...');

// ============================================================
// 25.1 FUNCIÓN PARA ESCUCHAR CAMBIOS EN FIRESTORE (CORREGIDA)
// ============================================================
...
function escucharCambiosFirebase() {
    ...
    const { collection, onSnapshot, query, orderBy } = window.firebaseHelpers;
    ...
    try {
        ...
        const qEventos = query(eventosRef, orderBy('id', 'desc'));
        
        const unsubEventos = onSnapshot(qEventos, (snapshot) => {
            console.log('📡 Eventos snapshot recibido, cambios:', snapshot.docChanges().length);
            
            snapshot.docChanges().forEach((change) => {
                const eventoData = change.doc.data();
                const eventoId = parseInt(eventoData.id);
                
                if (change.type === 'added') {
                    console.log('📅 Nuevo evento detectado:', eventoData.nombre);
                    
                    if (!leidos.eventos.has(eventoId) && !eventos.find(e => e.id === eventoId)) {
                        eventos.push({
                            id: eventoId,
                            nombre: eventoData.nombre || 'Nuevo evento',
                            categoria: 'Evento',
                            direccion: eventoData.direccion || '',
                            coordenadas: eventoData.coordenadas || [-30.8580, -64.5240],
                            telefono: eventoData.telefono || null,
                            whatsapp: eventoData.whatsapp || null,
                            instagram: eventoData.instagram || null,
                            facebook: eventoData.facebook || null,
                            twitter: eventoData.twitter || null,
                            website: eventoData.website || null,
                            delivery: false,
                            deliveryInfo: null,
                            horario: eventoData.horario || '',
                            descripcion: eventoData.descripcion || '',
                            metodoPago: [],
                            foto: eventoData.foto || '',
                            fecha: eventoData.fecha || 'Nuevo'
                        });
                        
                        notificacionesEstado.eventos++;
                        mostrarNotificacionToast('🔔 ¡Nuevas notificaciones!');
                    }
                } else if (change.type === 'modified') {
                    console.log('🔄 Evento modificado:', eventoData.nombre);
                    
                    const index = eventos.findIndex(e => e.id === eventoId);
                    if (index !== -1) {
                        eventos[index] = {
                            ...eventos[index],
                            nombre: eventoData.nombre || eventos[index].nombre,
                            descripcion: eventoData.descripcion || eventos[index].descripcion,
                            direccion: eventoData.direccion || eventos[index].direccion,
                            horario: eventoData.horario || eventos[index].horario,
                            foto: eventoData.foto || eventos[index].foto,
                            fecha: eventoData.fecha || eventos[index].fecha,
                            coordenadas: eventoData.coordenadas || eventos[index].coordenadas,
                            whatsapp: eventoData.whatsapp || eventos[index].whatsapp,
                            instagram: eventoData.instagram || eventos[index].instagram,
                            facebook: eventoData.facebook || eventos[index].facebook,
                            twitter: eventoData.twitter || eventos[index].twitter,
                            website: eventoData.website || eventos[index].website
                        };
                        
                        if (!leidos.eventos.has(eventoId)) {
                            notificacionesEstado.eventos++;
                        }
                    }
                } else if (change.type === 'removed') {
                    console.log('🗑️ Evento eliminado:', eventoData.nombre);
                    ...
                    if (index !== -1) {
                        ...
                        if (!leidos.eventos.has(eventoId)) {
                            notificacionesEstado.eventos = Math.max(0, notificacionesEstado.eventos - 1);
                        }
                        
                        leidos.eventos.delete(eventoId);
                        guardarLeidos();
                    }
                }
            });
            ...
            if (document.getElementById('seccion-eventos').style.display !== 'none') {
                renderizarEventos();
            }
        }, (error) => {
        ...
        // ===== ESCUCHAR NOVEDADES =====
        const novedadesRef = collection(window.firebaseDB, 'novedades');
        const qNovedades = query(novedadesRef, orderBy('id', 'desc'));
        
        const unsubNovedades = onSnapshot(qNovedades, (snapshot) => {
            console.log('📡 Novedades snapshot recibido, cambios:', snapshot.docChanges().length);
            
            snapshot.docChanges().forEach((change) => {
                const novedadData = change.doc.data();
                const novedadId = parseInt(novedadData.id);
                
                if (change.type === 'added') {
                    console.log('📰 Nueva novedad detectada:', novedadData.nombre);
                    
                    if (!leidos.novedades.has(novedadId) && !novedades.find(n => n.id === novedadId)) {
                        novedades.push({
                            id: novedadId,
                            nombre: novedadData.nombre || 'Nueva novedad',
                            categoria: 'Novedad',
                            direccion: novedadData.direccion || '',
                            coordenadas: novedadData.coordenadas || [-30.8580, -64.5240],
                            telefono: novedadData.telefono || null,
                            whatsapp: novedadData.whatsapp || null,
                            instagram: novedadData.instagram || null,
                            facebook: novedadData.facebook || null,
                            twitter: novedadData.twitter || null,
                            website: novedadData.website || null,
                            delivery: false,
                            deliveryInfo: null,
                            horario: novedadData.horario || null,
                            descripcion: novedadData.descripcion || '',
                            metodoPago: [],
                            foto: novedadData.foto || '',
                            fecha: novedadData.fecha || 'Nuevo'
                        });
                        
                        notificacionesEstado.novedades++;
                        mostrarNotificacionToast('🔔 ¡Nuevas notificaciones!');
                    }
                } else if (change.type === 'modified') {
                    console.log('🔄 Novedad modificada:', novedadData.nombre);
                    
                    const index = novedades.findIndex(n => n.id === novedadId);
                    if (index !== -1) {
                        novedades[index] = {
                            ...novedades[index],
                            nombre: novedadData.nombre || novedades[index].nombre,
                            descripcion: novedadData.descripcion || novedades[index].descripcion,
                            direccion: novedadData.direccion || novedades[index].direccion,
                            foto: novedadData.foto || novedades[index].foto,
                            fecha: novedadData.fecha || novedades[index].fecha
                        };
                        
                        if (!leidos.novedades.has(novedadId)) {
                            notificacionesEstado.novedades++;
                        }
                    }
                } else if (change.type === 'removed') {
                    console.log('🗑️ Novedad eliminada:', novedadData.nombre);
                    ...
                    if (index !== -1) {
                        ...
                        if (!leidos.novedades.has(novedadId)) {
                            notificacionesEstado.novedades = Math.max(0, notificacionesEstado.novedades - 1);
                        }
                        
                        leidos.novedades.delete(novedadId);
                        guardarLeidos();
                    }
                }
            });
            ...
            if (document.getElementById('seccion-novedades').style.display !== 'none') {
                renderizarNovedades();
            }
        }, (error) => {
            console.error('❌ Error en listener de novedades:', error);
        });

        // ===== ESCUCHAR NOTICIAS =====
        const noticiasRef = collection(window.firebaseDB, 'noticias');
        const qNoticias = query(noticiasRef, orderBy('id', 'desc'));
        
        const unsubNoticias = onSnapshot(qNoticias, (snapshot) => {
            console.log('📡 Noticias snapshot recibido, cambios:', snapshot.docChanges().length);
            
            snapshot.docChanges().forEach((change) => {
                const noticiaData = change.doc.data();
                const noticiaId = parseInt(noticiaData.id);
                
                if (change.type === 'added') {
                    console.log('📢 Nueva noticia detectada:', noticiaData.nombre);
                    
                    if (!leidos.noticias.has(noticiaId) && !noticias.find(n => n.id === noticiaId)) {
                        noticias.push({
                            id: noticiaId,
                            nombre: noticiaData.nombre || 'Nueva noticia',
                            categoria: 'Noticia',
                            direccion: noticiaData.direccion || '',
                            coordenadas: noticiaData.coordenadas || [-30.8580, -64.5240],
                            telefono: noticiaData.telefono || null,
                            whatsapp: noticiaData.whatsapp || null,
                            instagram: noticiaData.instagram || null,
                            facebook: noticiaData.facebook || null,
                            twitter: noticiaData.twitter || null,
                            website: noticiaData.website || null,
                            delivery: false,
                            deliveryInfo: null,
                            horario: noticiaData.horario || null,
                            descripcion: noticiaData.descripcion || '',
                            metodoPago: [],
                            foto: noticiaData.foto || '',
                            fecha: noticiaData.fecha || 'Nuevo'
                        });
                        
                        notificacionesEstado.noticias++;
                        mostrarNotificacionToast('🔔 ¡Nuevas notificaciones!');
                    }
                } else if (change.type === 'modified') {
                    console.log('🔄 Noticia modificada:', noticiaData.nombre);
                    
                    const index = noticias.findIndex(n => n.id === noticiaId);
                    if (index !== -1) {
                        noticias[index] = {
                            ...noticias[index],
                            nombre: noticiaData.nombre || noticias[index].nombre,
                            descripcion: noticiaData.descripcion || noticias[index].descripcion,
                            direccion: noticiaData.direccion || noticias[index].direccion,
                            foto: noticiaData.foto || noticias[index].foto,
                            fecha: noticiaData.fecha || noticias[index].fecha
                        };
                        
                        if (!leidos.noticias.has(noticiaId)) {
                            notificacionesEstado.noticias++;
                        }
                    }
                } else if (change.type === 'removed') {
                    console.log('🗑️ Noticia eliminada:', noticiaData.nombre);
                    ...
                    if (index !== -1) {
                        ...
                        if (!leidos.noticias.has(noticiaId)) {
                            notificacionesEstado.noticias = Math.max(0, notificacionesEstado.noticias - 1);
                        }
                        
                        leidos.noticias.delete(noticiaId);
                        guardarLeidos();
                    }
                }
            });
            ...
            if (document.getElementById('seccion-noticias').style.display !== 'none') {
                renderizarNoticias();
            }
        }, (error) => {
        ...
        window._firebaseListenerActivo = true;
        console.log('✅ Listeners de Firebase activados correctamente');
        return true;
...
// ============================================================
// 25.2 FUNCIÓN PARA AGREGAR CONTENIDO DESDE FIREBASE
// ============================================================

async function agregarContenidoFirebase(tipo, datos) {
    if (!window.firebaseDB || !window.firebaseHelpers) {
    ...
    try {
        const coleccion = collection(window.firebaseDB, tipo === 'evento' ? 'eventos' : 
                                              tipo === 'novedad' ? 'novedades' : 'noticias');
        ...
        console.log(`✅ ${tipo} agregado a Firebase correctamente`);
        return true;
    } catch (error) {
...
// ============================================================
// 25.3 FUNCIÓN PARA ACTUALIZAR CONTENIDO DESDE FIREBASE
// ============================================================

async function actualizarContenidoFirebase(tipo, id, nuevosDatos) {
    if (!window.firebaseDB || !window.firebaseHelpers) {
        console.error('❌ Firebase no está inicializado');
        return false;
    }
    ...
    try {
        const coleccion = tipo === 'evento' ? 'eventos' : 
                         tipo === 'novedad' ? 'novedades' : 'noticias';
        
        const ref = doc(window.firebaseDB, coleccion, String(id));
        await updateDoc(ref, nuevosDatos);
        
        console.log(`✅ ${tipo} con ID ${id} actualizado correctamente`);
        return true;
    } catch (error) {
...
// ============================================================
// 25.4 FUNCIÓN PARA ELIMINAR CONTENIDO DESDE FIREBASE
// ============================================================

async function eliminarContenidoFirebase(tipo, id) {
    if (!window.firebaseDB || !window.firebaseHelpers) {
        console.error('❌ Firebase no está inicializado');
        return false;
    }
...
// ============================================================
// 25.5 FUNCIÓN PARA FORZAR CONEXIÓN INMEDIATA
// ============================================================

function iniciarListenerInmediato() {
    console.log('🚀 Intentando iniciar listener inmediatamente...');
    
    // Verificar si Firebase está listo
    if (window.firebaseDB && window.firebaseHelpers) {
        console.log('✅ Firebase detectado, iniciando listener...');
        escucharCambiosFirebase();
        window._firebaseListenerActivo = true;
        return true;
    } else {
        console.log('⏳ Firebase no está listo, reintentando en 1 segundo...');
        setTimeout(iniciarListenerInmediato, 1000);
        return false;
    }
}

// Iniciar inmediatamente
iniciarListenerInmediato();

// Reintentar después de 5 segundos por si acaso
setTimeout(() => {
    if (!window._firebaseListenerActivo) {
        console.log('🔄 Reintentando conexión después de 5 segundos...');
        iniciarListenerInmediato();
    }
}, 5000);
...
setTimeout(() => {
    if (!window._firebaseListenerActivo) {
        console.log('🔄 Último intento después de 10 segundos...');
        iniciarListenerInmediato();
    }
}, 10000);

// ============================================================
// 25.6 FUNCIÓN DE DIAGNÓSTICO
// ============================================================
...
function verificarConexionFirebase() {
    ...
    if (!window.firebaseDB) {
        console.error('❌ Firebase NO está inicializado');
        console.log('📋 Revisa que el script de Firebase en index.html tenga la config correcta');
        return false;
    }
    
    if (!window.firebaseHelpers) {
        console.error('❌ Firebase Helpers NO están disponibles');
        return false;
    }
    
    console.log('✅ Firebase DB detectado:', !!window.firebaseDB);
    console.log('✅ Firebase Helpers detectados:', !!window.firebaseHelpers);
    
    if (window._firebaseListenerActivo) {
        console.log('✅ Listener de Firebase ACTIVO');
    } else {
        console.warn('⚠️ Listener de Firebase NO está activo');
    }
    
    return true;
}

// Llamar al diagnóstico después de 3 segundos
setTimeout(() => {
    verificarConexionFirebase();
}, 3000);

// ============================================================
// 25.7 INDICADOR VISUAL DE CONEXIÓN
// ============================================================

/*function mostrarEstadoFirebase(conectado) {
    let indicador = document.getElementById('firebaseStatus');
    
    if (!indicador) {
        indicador = document.createElement('div');
        indicador.id = 'firebaseStatus';
    ...
    if (conectado) {
        indicador.textContent = '🟢 Firebase Conectado';
        indicador.style.background = '#2E7D32';
        indicador.style.color = 'white';
    } else {
        indicador.textContent = '🔴 Firebase Desconectado';
...
setInterval(() => {
    if (window.firebaseDB && window._firebaseListenerActivo) {
        mostrarEstadoFirebase(true);
    } else {
        mostrarEstadoFirebase(false);
        // Reintentar conexión
        iniciarListenerInmediato();
    }
}, 5000);*/
...
// ============================================================
// 25.7 VERIFICACIÓN SILENCIOSA DE CONEXIÓN (SIN INDICADOR VISUAL)
// ============================================================

// Verificar conexión cada 10 segundos (solo en consola)
setInterval(() => {
    if (window.firebaseDB && window._firebaseListenerActivo) {
        console.log('✅ Firebase conectado y listener activo');
    } else {
        console.warn('⚠️ Firebase desconectado, reintentando conexión...');
        iniciarListenerInmediato();
    }
}, 10000);

// ============================================================
//  26. FUNCIONES DE PRUEBA PARA CONSOLA
// ============================================================

// Probar agregar un evento desde consola
window.testAgregarEvento = async function() {
    return await agregarContenidoFirebase('evento', {
        id: Date.now(),
        nombre: 'Evento de prueba ' + new Date().toLocaleTimeString(),
        categoria: 'Evento',
        descripcion: 'Este es un evento de prueba agregado desde la consola',
        direccion: 'Plaza Central',
        coordenadas: [-30.8580, -64.5240],
        fecha: 'Hoy',
        foto: 'https://picsum.photos/seed/test/400/200'
    });
};

// Probar agregar una novedad desde consola
window.testAgregarNovedad = async function() {
    return await agregarContenidoFirebase('novedad', {
        id: Date.now(),
        nombre: 'Novedad de prueba ' + new Date().toLocaleTimeString(),
        categoria: 'Novedad',
        descripcion: 'Esta es una novedad de prueba',
        fecha: 'Hoy',
        foto: 'https://picsum.photos/seed/test2/400/200'
    });
};

// Probar agregar una noticia desde consola
window.testAgregarNoticia = async function() {
    return await agregarContenidoFirebase('noticia', {
        id: Date.now(),
        nombre: 'Noticia de prueba ' + new Date().toLocaleTimeString(),
        categoria: 'Noticia',
        descripcion: 'Esta es una noticia de prueba',
        fecha: 'Hoy',
        foto: 'https://picsum.photos/seed/test3/400/200'
    });
};
...
window.verNotificaciones = function() {
    ...
    console.log('Eventos no leídos:', notificacionesEstado.eventos);
    console.log('Novedades no leídas:', notificacionesEstado.novedades);
    console.log('Noticias no leídas:', notificacionesEstado.noticias);
    console.log('Leídos guardados:', {
        eventos: leidos.eventos.size,
        novedades: leidos.novedades.size,
        noticias: leidos.noticias.size
...
console.log('  - testAgregarEvento() : Agregar evento de prueba');
console.log('  - testAgregarNovedad() : Agregar novedad de prueba');
console.log('  - testAgregarNoticia() : Agregar noticia de prueba');
console.log('  - verNotificaciones() : Ver estado');
