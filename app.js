...
// ============================================================
//  17. NOTIFICACIONES - SISTEMA COMPLETO CON BADGES ESTILO ANDROID
// ============================================================
...
function recalcularTodasLasNotificaciones() {
    ...
    console.log('📊 Notificaciones recalculadas:', notificacionesEstado);

    actualizarBadges();
}

function actualizarBadges() {
    console.log('🔄 Actualizando badges en menú y secciones...');

    // ===== BADGES EN MENÚ INFERIOR =====
    const menus = [
        { id: 'menuBadgeEventos', contador: notificacionesEstado.eventos, nombre: 'Eventos' },
        { id: 'menuBadgeNovedades', contador: notificacionesEstado.novedades, nombre: 'Novedades' },
        { id: 'menuBadgeNoticias', contador: notificacionesEstado.noticias, nombre: 'Noticias' }
    ];

    let totalNotificaciones = 0;

    menus.forEach(({ id, contador, nombre }) => {
        ...
        console.log(`🔔 Badge Menú ${nombre} (${id}): ${contador}`);
    ...
    const secciones = [
        { id: 'badgeEventos', contador: notificacionesEstado.eventos, nombre: 'Eventos' },
        { id: 'badgeNovedades', contador: notificacionesEstado.novedades, nombre: 'Novedades' },
        { id: 'badgeNoticias', contador: notificacionesEstado.noticias, nombre: 'Noticias' }
    ];

    secciones.forEach(({ id, contador, nombre }) => {
        const badge = document.getElementById(id);
        if (!badge) {
            console.warn(`⚠️ No se encontró el elemento ${id}`);
            return;
        ...
        console.log(`🔔 Badge Sección ${nombre} (${id}): ${contador}`);
    ...
    if (badgePrincipal) {
        if (totalNotificaciones > 0) {
            ...
            console.log(`🔔 Campana total: ${totalNotificaciones}`);
        } else {
            badgePrincipal.style.display = 'none';
            console.log('🔔 Campana: sin notificaciones');
        }
    }
}
...
function guardarLeidos() {
    try {
        const data = {};
        for (const key in leidos) {
            data[key] = Array.from(leidos[key]);
        }
...
function cargarLeidos() {
    try {
        ...
        for (const key in data) {
            if (leidos[key]) {
                data[key].forEach(id => leidos[key].add(id));
            }
        }
    } catch (e) {
...
// ============================================================
// RENDERIZAR SECCIONES CON RESETEO DE BADGES
// ============================================================
...
function renderizarEventos() {
    ...
    // MARCAR TODOS COMO LEÍDOS Y RESETEAR BADGE
    eventos.forEach(evento => marcarComoLeido('eventos', evento.id));
    guardarLeidos();
    notificacionesEstado.eventos = 0;
    actualizarBadges();
    console.log('✅ Eventos: todos marcados como leídos');
}
...
function renderizarNovedades() {
    ...
    actualizarBadges();
    console.log('✅ Novedades: todos marcados como leídos');
}
...
function renderizarNoticias() {
    ...
    actualizarBadges();
    console.log('✅ Noticias: todos marcados como leídos');
}
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // CALCULAR NOTIFICACIONES INICIALES
    setTimeout(() => {
...
// ============================================================
//  22. FORZAR ACTUALIZACIÓN DE NOTIFICACIONES AL CARGAR
// ============================================================

// Esto fuerza a que las notificaciones se calculen SIEMPRE,
// incluso si el navegador usa caché
(function forzarActualizacionNotificaciones() {
    console.log('🔄 Forzando actualización de notificaciones...');
    
    // Esperar a que el DOM esté listo
    if (document.readyState === 'complete') {
        // Si ya cargó todo, ejecutar inmediatamente
        setTimeout(() => {
            recalcularTodasLasNotificaciones();
            console.log('✅ Notificaciones actualizadas (carga completa)');
        }, 100);
    } else {
        // Si no cargó, esperar
        window.addEventListener('load', function() {
            setTimeout(() => {
                recalcularTodasLasNotificaciones();
                console.log('✅ Notificaciones actualizadas (evento load)');
            }, 100);
        });
    }
})();

// También forzamos cada vez que se recarga la página con F5
// Esto hace que el contador se actualice siempre
setInterval(() => {
    // Solo si la página está visible
    if (!document.hidden) {
        recalcularTodasLasNotificaciones();
    }
}, 5000); // Cada 5 segundos verifica si hay novedades