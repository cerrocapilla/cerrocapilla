...
function actualizarBadges() {
    ...
    const menuBadges = [
        { id: 'menuBadgeEventos', contador: notificacionesEstado.eventos },
        { id: 'menuBadgeNovedades', contador: notificacionesEstado.novedades },
        { id: 'menuBadgeNoticias', contador: notificacionesEstado.noticias }
    ];
    ...
    const seccionBadges = [
        { id: 'badgeEventos', contador: notificacionesEstado.eventos },
        { id: 'badgeNovedades', contador: notificacionesEstado.novedades },
        { id: 'badgeNoticias', contador: notificacionesEstado.noticias }
    ];
    ...
    if (badgePrincipal) {
        if (totalNotificaciones > 0) {
            ...
            console.log(`🔔 Campana: ${totalNotificaciones} notificaciones`);
        } else {
...
function cargarLeidos() {
    try {
        ...
        // Verificar que los datos existan y tengan el formato correcto
...
function renderizarEventos() {
    ...
    // PRIMERO: Marcar todos como leídos ANTES de renderizar
    ...
    actualizarBadges();
    console.log('✅ Eventos: marcados como leídos, badge reseteados');
...
function renderizarNovedades() {
    ...
    actualizarBadges();
    console.log('✅ Novedades: marcados como leídos, badge reseteados');
...
function renderizarNoticias() {
    ...
    actualizarBadges();
    console.log('✅ Noticias: marcados como leídos, badge reseteados');
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    // PRIMERO: CARGAR DATOS GUARDADOS (NO LIMPIAR)
    cargarLeidos();
    cargarConfigNotificaciones();
    ...
    // CALCULAR NOTIFICACIONES INICIALES (DESPUÉS DE CARGAR LEÍDOS)
    setTimeout(() => {
...
// Función para debug - muestra el estado actual
...
// Función para forzar la visualización de badges desde consola
...
// Función para resetear TODOS los leídos (solo para pruebas)
...
// Inicialización - NO limpiar los datos existentes
...
setTimeout(() => {
    ...
    // Recalcular para actualizar badges
    recalcularTodasLasNotificaciones();
}, 1000);
...
// ============================================================
// 23.1 AJUSTAR BADGES PARA MÓVILES
// ============================================================

// Función que ajusta el tamaño de los badges según el dispositivo
function ajustarBadgesParaMovil() {
    const isMobile = window.innerWidth <= 768;
    const badgeSize = isMobile ? '16px' : '20px';
    const fontSize = isMobile ? '9px' : '11px';
    const topPosition = isMobile ? '-6px' : '-8px';
    const rightPosition = isMobile ? '-6px' : '-8px';
    ...
    badges.forEach(id => {
        const badge = document.getElementById(id);
        if (badge) {
            badge.style.minWidth = badgeSize;
            badge.style.height = badgeSize;
            badge.style.fontSize = fontSize;
            badge.style.top = topPosition;
            badge.style.right = rightPosition;
            badge.style.padding = isMobile ? '0 3px' : '0 5px';
        }
    });
    
    // Ajustar badge de la campana
    const campana = document.getElementById('notificacionesBadge');
    if (campana) {
        campana.style.minWidth = isMobile ? '14px' : '18px';
        campana.style.height = isMobile ? '14px' : '18px';
        campana.style.fontSize = isMobile ? '8px' : '10px';
        campana.style.top = isMobile ? '-4px' : '-5px';
        campana.style.right = isMobile ? '-4px' : '-5px';
        campana.style.padding = isMobile ? '0 2px' : '0 4px';
    }
}
...
// También ejecutar después de actualizar badges
...
console.log('✅ Badges ajustados para móviles');

// ============================================================
// 23.2 HACER CLICKEABLE LOS ITEMS DE LUGARES CERCANOS
// ============================================================

// Función que busca un lugar por nombre y lo muestra
function buscarYMostrarLugar(nombre) {
    console.log(`🔍 Buscando lugar: "${nombre}"`);
    
    // Buscar en la lista de lugares (case insensitive)
    ...
    if (lugarEncontrado) {
        ...
        // Cerrar el modal de lugares cercanos
        const modal = document.getElementById('modalCercanos');
        if (modal) {
            modal.classList.remove('active');
            setTimeout(() => {
...
// Modificar la función mostrarListaCercanos para que los items sean clickeables
const mostrarListaCercanosOriginal = mostrarListaCercanos;
mostrarListaCercanos = function(cercanos) {
    // Llamar a la función original para generar el HTML
    mostrarListaCercanosOriginal(cercanos);
    
    // Ahora agregar event listeners a todos los items de lugares cercanos
    setTimeout(() => {
...
// También hacer clickeables los items existentes si ya hay un modal abierto
function hacerClickeablesItemsExistentes() {
    const items = document.querySelectorAll('.item-lugar');
    items.forEach(item => {
        // Solo si no tiene event listener (verificar si tiene onclick)
        if (!item._clickable) {
...
// Observar cambios en el DOM para hacer clickeables los nuevos items
