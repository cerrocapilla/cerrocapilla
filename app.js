...
console.log('⏰ Activando actualización automática de notificaciones...');

// Variable para controlar si ya se mostró una notificación nueva
let ultimoEstadoNotificaciones = {
    eventos: 0,
    novedades: 0,
    noticias: 0
};
...
function verificarCambiosNotificaciones() {
    ...
    const estadoAnterior = { ...notificacionesEstado };
    ...
    if (huboCambios) {
        console.log('🔄 Cambios detectados en notificaciones:', notificacionesEstado);
        
        // Verificar si aumentó alguna notificación
...
function restaurarEstadoNotificaciones() {
    try {
        ...
        if (guardado) {
            ...
            if (Date.now() - estado.timestamp < 300000) { // 5 minutos
...
// Iniciar la verificación automática cada 5 segundos
setInterval(() => {
...
// También verificar cuando la página vuelve a ser visible
document.addEventListener('visibilitychange', () => {
    if (!document.hidden) {
        console.log('👁️ Página visible, verificando notificaciones...');
        verificarCambiosNotificaciones();
    }
});
...
console.log('✅ Actualización automática de notificaciones activada (cada 5 segundos)');
console.log('💡 Las notificaciones se actualizarán automáticamente sin recargar la página');
